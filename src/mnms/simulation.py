...
class Supervisor(object):
    ...
    def call_matching_mobility_services(self, new_users, flow_dt):
        ...
        for layer in self._mlgraph.layers.values():
            for ms in layer.mobility_services.values():
                # Force request cancellation for users that have been replaned and don't use this ms anymore
                for uid, req in list(ms.user_buffer.items()):
                    user = req.user
                    if user.requested_service != ms:
                        ms.cancel_request(uid)
    ...
    def run(self,
            ...
            update_graph_threshold: float = 0.,
            seed: int=None):
        ...
        Args:
            ...
            -static_pt_load_estimation_file: path of json file where info is stored for statically estimating load in PT vehicles
            -expected_pt_share: estimated share of public transport among trips
        """
        ...
        # f = open(f'OUTPUTS/congestion_file.csv', 'w') # TODO : changer le chemin du fichier en fonction du dossier
        ...
        while self.tcurrent < tend:
            ...
            for _ in range(affectation_factor):
                ...
                self.call_planning() # computes k shortest paths for users anc chooses one
                ...
                # Call user flow step
                users_reach_dt_answer = self.call_user_flow_step(flow_dt, users_step) # list of users who undergone a match failure
                ...
                users_to_replan = self.call_flow_motor_step(flow_dt) # c'est ici que les véhicules bougent

...
# from mnms.simulation import find_zone_interval_direction
...
class PublicTransportMobilityService(AbstractMobilityService):
    def __init__(self, id: str,
                 veh_capacity: int = 50,
                 capacity_info=None,
                 demand_analysis_file: str = None,
                 estimate_pt_load_method: str = None,
        ...
        self.demand_analysis_file = demand_analysis_file
        self.estimate_pt_load_method = estimate_pt_load_method
    ...
    def new_departures(self, time, dt, lid: str):
        ...
        if self._next_veh_departure[lid] is None and self._current_time_table[lid] is not None:
            ...
            capacity = self._veh_capacity #self.capacity_info[lid] # TODO
    ...
    def new_departures_recursive(self, time, dt, lid: str, all_departures=None):
        ...
        if self._current_time_table[lid] is not None and (time <= self._current_time_table[lid] < next_time):
            ...
            try:
                ...
                capacity = self._veh_capacity #self.capacity_info[lid]
    ...
    def add_passenger(self, user: User, drop_node: str, veh: Vehicle, line_nodes: List[str]):
        ...
        # manage duplicates in nodes list
        if do_node_ind < pu_node_ind:
            do_node_ind = line_nodes[pu_node_ind:].index(drop_node)
            if user.current_node in line_nodes[pu_node_ind+1:]:
                pu_node_ind = line_nodes[pu_node_ind+1:].index(user.current_node)
    ...
    def find_line(self, node):
        ...
        """
        #if 'U598' in self.user_buffer.keys():
            #breakpoint()
    ...
    def estimate_pickup_time_for_planning(self, pu_node, **kwargs):
        """
        ...
        headway = chosen_line['table'].get_freq() # method get_freq() actually returns the headway and not the frequency
        estimated_pickup_time = headway/2

        user = kwargs.get('user', None)

        if self.estimate_pt_load_method == 'dynamic': # demand_analysis_file is None

            # Get load of vehicles that left pu_node in time_span and estimate number of passengers in next vehicle
            previous_loads = self._observer.get_load_of_vehicles_at_node(pu_node, t_min=self._tcurrent.remove_time(Dt(hours=1)), t_max=self._tcurrent)
            estimated_passengers, estimation_time, previous_load = self.estimate_number_of_passengers_dynamic(previous_loads, headway)

            if estimated_passengers >= self.load_threshold * self.veh_capacity:
                k = ceil(estimated_passengers/(self.load_threshold * self.veh_capacity))
                estimated_pickup_time += k * headway

        elif self.estimate_pt_load_method == 'static': # demand_analysis_file is not None
            headway_factor = self.estimate_waiting_time_static(pu_node, user)
            estimated_pickup_time += headway * headway_factor

        elif self.estimate_pt_load_method == 'static_distrib':  # demand_analysis_file is not None
            headway_factor = self.estimate_waiting_time_static_distrib(pu_node, user)
            estimated_pickup_time += headway * headway_factor

        elif self.estimate_pt_load_method == 'debug':
            if self._tcurrent > user.departure_time: # si l'usager est en replanification
                if 'bus' in pu_node: # le forcer à prendre le bus en augmentant le coût du tram
                    estimated_pickup_time = 100000
            else:
                if 'tram' in pu_node:
                    estimated_pickup_time = 100000
    ...
    def estimate_waiting_time_static(self, pu_node, user):
        """
        TODO : explications
        """
        zones = self.demand_analysis_file
        z, t, d = find_zone_interval_direction(zones, self.gnodes[pu_node].position, user.destination, self._tcurrent, static_distrib=False)
        est_nb_headways = zones[z][d]["demand"][t]["waiting_factor"]
        return est_nb_headways

    def estimate_waiting_time_static_distrib(self, pu_node, user):
        """
        TODO : explications
        """
        waiting_matrix = self.demand_analysis_file
        z, tau, theta = find_zone_interval_direction(waiting_matrix, self.gnodes[pu_node].position, user.destination, self._tcurrent, static_distrib=True)
        est_nb_headways = waiting_matrix[z][theta][tau]
        return est_nb_headways

    def estimate_number_of_passengers_dynamic(self, previous_loads, headway):
        """
        Estimate the number of passengers in the next vehicle at a stop, based on the number of passengers in last vehicles at this stop
        Args:
            previous_loads: list of dicts of the form
                {"id": id of the vehicle, "time": time at which vehicle left stop, "passengers": passengers on board when vehicle left stop}
                must be sorted by ascending value of "time"
            headway: headway between two vehicles (seconds)
        """
        if len(previous_loads) == 0:
            # if no vehicle in the last hour, consider no passenger in the next one
            # could be replaced by an estimation based on the nb of departures around the node
            return 0, None, 0
        elif len(previous_loads) < 3:
            # if less than 3 vehicles in the last hour, then consider constant load
            prediction = int(previous_loads[-1]["passengers at node"])
            next_time = (previous_loads[-1]["time"].add_time(Dt(seconds=headway))).to_seconds()
            return prediction, next_time, previous_loads[-1]["passengers at node"]
        else :
            # if more than 3 vehicles in the last hour, use linear regression on the last 3
            previous_loads = previous_loads[-3:]
            TIMES = np.array([d["time"].to_seconds() for d in previous_loads]).reshape(-1, 1)
            PASSENGERS = np.array([d["passengers at node"] for d in previous_loads])

            model = LinearRegression()
            model.fit(TIMES, PASSENGERS)

            next_time = (previous_loads[-1]["time"].add_time(Dt(seconds=headway))).to_seconds() # could be replaced by the estimated arrival time of future user at the stop
            prediction = int(model.predict(np.array([[next_time]]))[0])
...
def find_zone_interval_direction(zones_dict, origin, destination, departure_time, static_distrib):
    z, t, d = None, None, None
    ...
    if static_distrib:
        intervals_list = list(zones_dict[z][d].keys())
    else:
        intervals_list = list(zones_dict[z][d]["demand"].keys())

    for i in intervals_list:
        tmin, tmax = Time(i.split(' ')[0]), Time(i.split(' ')[1])
        if tmin <= departure_time < tmax:
            t = i

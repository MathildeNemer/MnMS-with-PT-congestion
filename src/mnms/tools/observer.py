...
class CSVVehicleObserver(TimeDependentObserver):
    ...
    def update(self, subject: 'Vehicle', time:Time):
        ...
        if len(list(subject._achieved_path)) > 0:
            last_node = list(subject._achieved_path)[-1]
            new_elt = {
                    "time": time,
                    "veh id": subject.id,
                    "passengers at node": len(subject.passengers)
                     }

            if last_node in self._memory_of_load:
                self._memory_of_load[last_node].append(new_elt)
            else:
                self._memory_of_load[last_node] = [new_elt]

    def get_load_of_vehicles_at_node(self, pu_node, t_min:Time, t_max:Time):
        """
        Return load of vehicles that left pu_node in [t_min, t_max]

        Args:
            pu_node: considered pick up node
            t_min, t_max: time frame
        """
        previous_loads = []
        seen_ids = set()
        try :
            for entry in self._memory_of_load[pu_node]: # entry looks like {"id": id, "time": time, "passengers": passengers}
                if t_min <= entry["time"] <= t_max and entry["veh id"] not in seen_ids:
                    previous_loads.append(entry)
                    seen_ids.add(entry["veh id"])
        except KeyError:
            pass

        # Sort list of dicts by ascending value of "time"
        previous_loads = sorted(previous_loads, key=lambda x: x["time"])
        return previous_loads
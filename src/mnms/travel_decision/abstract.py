...
class AbstractDecisionModel(ABC):
    ...
    def treat_path(self, p, chosen_mservice, user, gnodes):
        ...
        synthese_wt_by_mode = {}
        estim_waiting_time = 0
        for (layer, node_inds), service in zip(p.layers, p.mobility_services) :
            if service != 'WALK':
                estim_waiting_time += sum([self._mlgraph.layers[layer].mobility_services[
                                              service].estimate_pickup_time_for_planning(p.nodes[node_inds][0], user=user)])
                synthese_wt_by_mode[service] = estim_waiting_time
            #else:
                #estim_waiting_time = 0

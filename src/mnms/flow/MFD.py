...
class MFDFlowMotor(AbstractMFDFlowMotor):
    ...
    def move_veh(self, veh: Vehicle, tcurrent: Time, dt: float, speed: float):
        ...
        users_to_replan = OrderedSet() # set()
        if dist_travelled > veh.remaining_link_length:
            ...
            except StopIteration: # reached end of activity path
                veh._current_node = veh.current_link[1]

                # If vehicle can take next activity, then:
                if len(veh.passengers) < veh.capacity or veh.activity_type != ActivityType.PICKUP:
                    veh.next_activity(tcurrent)

                # Else, if users can't board the vehicle:
                elif len(veh.passengers) >= veh.capacity and veh.activity_type == ActivityType.PICKUP:
                    # Has been modified to replan all overflowing users during the same step
                    users_over_capa = [veh.activity.user] + [a.user for a in veh.activities if a.activity_type == ActivityType.PICKUP and a.user is not None]
                    users_to_replan.update(users_over_capa)

                    # Find activities related to users to replan
                    to_remove = []
                    for a in veh.activities:
                        if a.user in users_over_capa and a.user is not None:
                            to_remove.append(a)

                    # Save paths but remove activities
                    for a in to_remove:
                        path = a.path
                        ind_next = veh.activities.index(a) + 1 # activity to modify adding the path. The next activity always exist since there are stop or serving final activities
                    ...
                    # for user in users_to_replan:
                    #     user.requested_service = None
    ...
    def step(self, dt: Dt):
        ...
        for veh_id, veh in self.veh_manager._vehicles.items():
            ...
            while veh.activity.is_done:
                ...
                if len(veh.passengers) < veh.capacity or veh.activity_type != ActivityType.PICKUP:
                    ...
                    # Has been modified to replan all overflowing users during the same step
                    users_over_capa = [veh.activity.user] + [a.user for a in veh.activities if
                                                             a.activity_type == ActivityType.PICKUP and a.user is not None]
                    users_to_replan.update(users_over_capa)
                    ...
                    for a in to_remove:
                        path = a.path
                        ind_next = veh.activities.index(
                            a) + 1  # activity to modify adding the path. The next activity always exist since there are stop or serving final activities
                        veh.activities[ind_next].modify_path(path + veh.activities[ind_next].path)
                        veh.activities.remove(a)
        ...
        for veh_id, veh in current_vehicles.items():
            ...
            while veh_dt > 0:
                ...
                users_to_replan = users_to_replan.union(other_users_to_replan) # "other_users_to_replan" added to the end of ordered set "users_to_replan" if not already in it

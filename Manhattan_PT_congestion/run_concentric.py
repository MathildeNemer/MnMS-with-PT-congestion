...
if __name__=='__main__':
    ...
    ### CONCENTRIC - WITH CAR TRAFFIC ###
    car_traffic = False
    if car_traffic:
        SCENARIO = f"Concentric"
        ...
        for n_users in [1300]:
            DEMAND_PARAMS = {"N_USERS": n_users,
                             ...
                             "D_MU": (3000, 3000),
                             "D_SIGMA": (400, 400),
                             "D_SIGMA_ORIGINS": 3000,
                             "DEMAND_SHAPE": 'unicentric'
                             }
            DEMAND_PARAMS[
                "DEMAND_SPECIFIER"] = f'{SCENARIO}_{DEMAND_PARAMS["DEMAND_T_BOUNDARIES"][0][:2]}-{DEMAND_PARAMS["DEMAND_T_BOUNDARIES"][-1][:2]}_peakhour={DEMAND_PARAMS["T_MU"][:5]}-{int(DEMAND_PARAMS["T_SIGMA"] // 60)}min_'

            for estim_method in ['none', 'dynamic', 'static']:
    ...
    ### CONCENTRIC - ALL METHODS - EFFECTS OF PT CONGESTION NO CAR - LT=1 ###
    basic = True
    if basic:
        SCENARIO = "Concentric"
    ...
    ### CONCENTRIC - ALL METHODS - SHARP PEAK HOUR - LT according to n_users ###
    sharp_peak = True
    if sharp_peak:
        SCENARIO = "Concentric"
        ...
        for n_users in [1300, 2600]:
            ...
            ## Other
            OTHER_SPECIFIER = f'_sharppeak21janv'
